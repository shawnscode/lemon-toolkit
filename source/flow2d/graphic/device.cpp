...
static const char* get_opengl_error(GLenum err)
{
    switch(err) {
...
struct Program
{
    ...
    int             texture_n;
    int             textures[kGfxMaxTextures];

    int             uniform_n;
    int             uniforms[kGfxMaxUniforms];
...
struct RenderState
{
    Rid             target;
    Rid             program;
    Rid             textures[kGfxMaxTextures];

    BufferLayout    vertex_buffers[kGfxMaxAttributes];
...
void GraphicDeviceInstance::reset()
{
    this->change_flags = ~0;
...
void GraphicDeviceInstance::commit()
{
...
void GraphicDeviceInstance::apply_target()
{
...
void GraphicDeviceInstance::apply_program()
{
...
void GraphicDeviceInstance::apply_vertex_array()
{
...
void GraphicDeviceInstance::apply_textures()
{
    auto program = array_get(this->programs, this->current.program);
    if( program == nullptr || program->handle == 0 ) return;
...
void GraphicDeviceInstance::apply_blend()
{
    static GLenum blend[] = {
        0,
        GL_ZERO,
...
void GraphicDeviceInstance::apply_depth()
{
    static GLenum depth[] = {
        0,
        GL_LEQUAL,
...
void GraphicDeviceInstance::apply_cull()
{
...
void GraphicDeviceInstance::apply_scissor()
{
    if (this->last.scissor != this->current.scissor) {
        ...
        glScissor(this->current.scissor_x, this->current.scissor_y, this->current.scissor_width, this->current.scissor_height);
        this->last.scissor = this->current.scissor;
    }
}

//// GLOBAL RENDER SINGLETON 
static GraphicDevice* s_instance = nullptr;
...
bool GraphicDevice::initialize()
{
    ...
    s_instance->m_state = new (std::nothrow) GraphicDeviceInstance();
...
void GraphicDevice::set_viewport(int x, int y, int width, int height)
{
    glViewport(x, y, width, height);
}

void GraphicDevice::set_scissor(bool enable, int x, int y, int width, int height)
{
    m_state->current.scissor = enable;
    m_state->current.scissor_x = x;
    m_state->current.scissor_y = y;
    m_state->current.scissor_width  = width;
    m_state->current.scissor_height = height;
    m_state->change_flags |= CHANGE_SCISSOR;
}

void GraphicDevice::set_blend(BlendFunc src, BlendFunc dst)
{
...
void GraphicDevice::set_depth(bool write, DepthTestFunc format)
{
...
void GraphicDevice::set_cull(CullMode mode)
{
...
void GraphicDevice::clear(uint32_t mask, uint8_t r, uint8_t g, uint8_t b, uint8_t a)
{
...
void GraphicDevice::bind_index_buffer(Rid id, ElementFormat format, size_t stride, size_t offset)
{
...
void GraphicDevice::bind_shader(Rid id)
{
...
Rid GraphicDevice::create_shader(
...
void GraphicDevice::update_buffer(RenderObject what, Rid id, const void* data, size_t size)
{
...
Rid GraphicDevice::create_texture(const void* data, size_t width, size_t height, TextureFormat format, int mipmap)
{
    ...
    glActiveTexture(GL_TEXTURE0+kGfxMaxTextures);
    glBindTexture(GL_TEXTURE_2D, texture->handle);
...
void GraphicDevice::release(RenderObject what, Rid id)
{
    switch(what)
    {
