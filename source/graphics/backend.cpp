...
{
    _device.get_dispatcher().subscribe<EvtBackendLost>(*this);
    _device.get_dispatcher().subscribe<EvtBackendRestore>(*this);
}

GraphicsObject::~GraphicsObject()
{
    _device.get_dispatcher().unsubscribe<EvtBackendLost>(*this);
    _device.get_dispatcher().unsubscribe<EvtBackendRestore>(*this);
}
...
bool Backend::restore_context()
{
    ...
    //
    get_dispatcher().emit<EvtBackendRestore>();
    return true;
}

void Backend::release_context()
{
    get_dispatcher().emit<EvtBackendLost>();
...
void Backend::set_cull_face(bool enable, CullFace face)
{
    ...
    {
        if( enable ) glCullFace(GL_CULL_FACE_FUNC[to_value(face)]);
...
void Backend::set_front_face(FrontFaceOrder winding)
{
    ...
    {
        glFrontFace(GL_FRONT_FACE_FUNC[to_value(winding)]);
...
void Backend::set_stencil_test(bool enable, CompareEquation equation, unsigned reference, unsigned mask)
{
    ...
    {
        if( enable ) glStencilFunc(GL_COMPARE_FUNC[to_value(equation)], reference, mask);
...
void Backend::set_stencil_write(StencilWriteEquation sfail, StencilWriteEquation dpfail, StencilWriteEquation dppass, unsigned mask)
{
    ...
    {
        glStencilOp(GL_STENCIL_OP[to_value(sfail)], GL_STENCIL_OP[to_value(dpfail)], GL_STENCIL_OP[to_value(dppass)]);
...
void Backend::set_depth_test(bool enable, CompareEquation equation)
{
    ...
    {
        if( enable ) glDepthFunc(GL_COMPARE_FUNC[to_value(equation)]);
...
void Backend::set_color_blend(bool enable, BlendEquation equation, BlendFactor src, BlendFactor dst)
{
    ...
    {
        glBlendFunc(GL_BLEND_FACTOR[to_value(src)], GL_BLEND_FACTOR[to_value(dst)]);
        glBlendEquation(GL_BLEND_EQUATION_FUNC[to_value(equation)]);
...
void Backend::set_color_write(ColorMask mask)
{
    ...
    {
        GLboolean r = to_value(mask & ColorMask::RED) > 0 ? GL_TRUE : GL_FALSE;
        GLboolean g = to_value(mask & ColorMask::GREEN) > 0 ? GL_TRUE : GL_FALSE;
        GLboolean b = to_value(mask & ColorMask::BLUE) > 0 ? GL_TRUE : GL_FALSE;
        GLboolean a = to_value(mask & ColorMask::ALPHA) > 0 ? GL_TRUE : GL_FALSE;
        glColorMask(r, g, b, a);
