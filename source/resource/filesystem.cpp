...
bool remove(const Path& path, bool recursive)
{
    ...
    if( is_directory(path) && recursive )
    {
        ...
        {
#ifdef PLATFORM_WIN32
            auto wstring_path = to_win32(iter.c_str());
            success &= DeleteFileW(wstring_path.c_str());
#else
            success &= (std::remove(iter.c_str()) == 0);
#endif
        }
...
std::fstream open(const Path& path, FileMode mode)
{
    std::fstream fs;
#ifdef PLATFORM_WIN32
    fs.open(to_win32(path.c_str()).c_str(), get_filemode(mode));
#else
    fs.open(path.c_str(), get_filemode(mode));
#endif

    if( !fs.is_open() )
        LOGW("failed to open file \"%s\" with mode %d",
            path.c_str(),(uint16_t)mode);
...
Directory scan(const Path& path, ScanMode mode)
{
    ...
    if( !directory.scan(path, mode) )
        LOGW("failed to scan directory \"%s\"", path.c_str());
    return directory;
}
